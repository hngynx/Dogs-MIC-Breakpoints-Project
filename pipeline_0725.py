...
def excel_formula_pipeline(input_csv, organism_col, start_col, output_csv):
    ...
    for col in antibiotic_cols: 
        if col == "Ampicillin":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_ampicillin(row[organism_col], row[col]), axis=1)
        elif col == "Amoxi/Clav":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_amoxiclav(row[organism_col], row[col]), axis=1)
        elif col == "Amikacin":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_amikacin(row[organism_col], row[col]), axis=1)  
        elif col == "Cefazolin":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_cefazolin(row[organism_col], row[col]), axis=1)
        elif col == "Cefovecin":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_cefovecin(row[organism_col], row[col]), axis=1)
        elif col == "Cefpodoxime":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_cefpodoxime(row[organism_col], row[col]), axis=1)
        elif col == "Ceftazidime":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_ceftazidime(row[organism_col], row[col]), axis=1)
        elif col == "Chloramphenicol":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_chloramphenicol(row[organism_col], row[col]), axis=1)
        elif col == "Doxycycline":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_doxycycline(row[organism_col], row[col]), axis=1)  
        elif col == "Gentamicin":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_gentamicin(row[organism_col], row[col]), axis=1)
        elif col == "Imipenem":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_imipenem(row[organism_col], row[col]), axis=1)
        elif col == "Minocycline":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_minocycline(row[organism_col], row[col]), axis=1)
        elif col == "Nitrofurantoin":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_nitrofurantoin(row[organism_col], row[col]), axis=1)
        elif col == "Orbifloxacin":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_orbifloxacin(row[organism_col], row[col]), axis=1)            
        elif col == "Piper/Tazob":  
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_pipertazobactam(row[organism_col], row[col]), axis=1)
        elif col == "Pradofloxacin":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_pradofloxacin(row[organism_col], row[col]), axis=1)
        elif col == "Tetracycline":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_tetracycline(row[organism_col], row[col]), axis=1)
        elif col == "Trim/Sulfa":
            df[col + "_RSI"] = df.apply(lambda row: determine_rsi_trimsulfa(row[organism_col], row[col]), axis=1)
